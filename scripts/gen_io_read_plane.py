...
for i in range(nformat):
    ...
    if (i==2 or i==3):
        ...
        f.write("            allocate (tmp(nplanes, decomp%xsz(2), decomp%xsz(3)))\n")
        f.write("         else if (ipencil == 2) then\n")
        f.write("            allocate (tmp(decomp%ysz(1), nplanes, decomp%ysz(3)))\n")
        f.write("         else if (ipencil == 3) then\n")
        f.write("            allocate (tmp(decomp%zsz(1), decomp%zsz(2), nplanes))\n")
        f.write("         end if\n")
        f.write("         call read_plane(ipencil, varname, decomp, nplanes, &\n")
        f.write("                         opt_dirname=opt_dirname, &\n")
        f.write("                         opt_mpi_file_open_info=opt_mpi_file_open_info, &\n")
