...
for i in range(nformat):
    ...
    if (i==0 or i==1):
        ...
        f.write("               var2d(1, :, :) = sum(var(:, :, :), dim=opt_ipencil)\n")
        f.write("            else\n")
    ...
    if (i==2 or i==3):
        ...
        f.write("            if (iplane < 1) then\n")
        if (i==2):
            f.write("               var2dbis(1, :, :) = real(sum(var(:, :, :), dim=opt_ipencil), kind=real32)\n")
        else:
            f.write("               var2dbis(1, :, :) = cmplx(sum(var(:, :, :), dim=opt_ipencil), kind=real32)\n")
        f.write("            else\n")
        if (i==2):
