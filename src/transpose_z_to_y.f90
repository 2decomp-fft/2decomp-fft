...
contains
   ...
   module subroutine transpose_z_to_y_real_long(src, dst, decomp)
      ...
      if (dims(2) == 1) then
         ...
         !$omp target teams distribute parallel do collapse(3) map(present,alloc:src,dst)
         do k = 1, size(src, 3)
            do j = 1, size(src, 2)
               do i = 1, size(src, 1)
                  dst(i, j, k) = src(i, j, k)
...
#   if defined(_GPU)
         ...
         if (istat /= 0) call decomp_2d_abort(__FILE__, __LINE__, istat, "cudaMemcpy")
#   elif defined(_OPENMP_GPU)
         !$omp target teams distribute parallel do private(pos) collapse(3) map(present,alloc:src,wk1)
         do k = 1, s3
            do j = 1, s2
               do i = 1, s1
                  pos = i + (j - 1) * s1 + (k - 1) * s1 * s2
                  wk1(pos) = src(i, j, k)
               end do
            end do
         end do
...
#   endif
#endif
      d2d_t_z2y_pack = d2d_t_z2y_pack + (MPI_WTIME() - t0)

      ! define receive buffer
      t0 = MPI_WTIME()
      d2d_n_z2y_mpi_calls = d2d_n_z2y_mpi_calls + 1
#ifdef EVEN
      if (decomp%even) then
...
#       if defined(_OPENMP_GPU)
         d2d_n_z2y_use_device_ptr = d2d_n_z2y_use_device_ptr + 1
...
#  endif
#endif
      d2d_t_z2y_mpi = d2d_t_z2y_mpi + (MPI_WTIME() - t0)

      ! rearrange receive buffer
      t0 = MPI_WTIME()
      call mem_merge_zy_real(wk2, d1, d2, d3, dst, dims(2), &
                             decomp%y2dist, decomp)
      d2d_t_z2y_unpack = d2d_t_z2y_unpack + (MPI_WTIME() - t0)
   ...
   subroutine transpose_z_to_y_complex(src, dst, decomp, wk1, wk2, wk1_r, wk2_r)
